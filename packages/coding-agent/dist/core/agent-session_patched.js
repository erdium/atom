...
import { createAllToolDefinitions, createReadUrlToolDefinition, createWebSearchToolDefinition, } from "./tools/index.js";
...
export class AgentSession {
        ...
        this.agent.beforeToolCall = async ({ toolCall, args }) => {
            ...
            if (toolCall.name === "bash") {
                ...
                if (command && isDangerousCommand(command)) {
                    ...
                    // Show confirmation dialog using runner's context
                    const ctx = runner.createContext();
                    if (ctx.hasUI) {
                        const choice = await ctx.ui.select(`Requesting permission for:\n\n   ${command}\n\nDo you want to proceed?`, ["Yes", "No"]);
                        if (choice !== "Yes") {
                            throw new Error(`Command blocked by user: ${command}`);
                        }
                    }
                    return undefined;
                }
     ...
     */
    setActiveToolsByName(toolNames) { console.error("ACTIVE TOOL NAMES:", JSON.stringify(toolNames)); console.error("TOOL REGISTRY KEYS:", JSON.stringify([...this._toolRegistry.keys()]));
        const tools = [];
    ...
    _buildRuntime(options) {
        ...
        this._baseToolDefinitions.set("web_search", createWebSearchToolDefinition());
        this._baseToolDefinitions.set("read_url", createReadUrlToolDefinition());
